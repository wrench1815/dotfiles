...
local WIDGET_DIR = HOME .. '~/.config/awesome/widgets/batteryarc'
...
local function worker(user_args)
    ...
    local font = args.font or 'Rubik 6'
    ...
    local main_color = args.main_color or '#ECEFF4'
    local bg_color = args.bg_color or '#2E3440'
    local low_level_color = args.low_level_color or '#BF616A'
    local medium_level_color = args.medium_level_color or '#D08770'
    local charging_color = args.charging_color or '#A3BE8C'
    ...
    local function update_widget(widget, stdout)
        ...
        for s in stdout:gmatch("[^\r\n]+") do
            local cur_status, charge_str, _ = string.match(s,
                                                           '.+: ([%a%s]+), (%d?%d?%d)%%,?(.*)')
        ...
        if status == 'Charging' then
            ...
            text_with_background.fg = '#2E3440'
        else
